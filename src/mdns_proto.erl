...
-moduledoc """
Helpers for building/reading the `#dns_rec{}` terms that `inet_dns`
...
cross into/out of `inet_dns`: its vendored record shapes
(`mdns_dns.hrl`) require a plain Erlang `string()` for any
domain-name-shaped field on encode, and always hand one back on
decode - as a list of raw wire bytes, not Unicode codepoints. TXT
record strings are the one exception on encode: `inet_dns` accepts
binaries for those directly, so only the decode side needs converting
back to this app's binary form.
""".
...
-doc "Lowercase Name and strip a trailing `.` - DNS names are case-insensitive; used as the cache/registry key everywhere in this app.".
...
-doc "Whether Name (already `normalize_name/1`'d) is `.local` or exactly `local` itself.".
...
-doc """
Build an mDNS question packet (as a `#dns_rec{}`) asking for Type
records of Name. A plain (non-QU) question: we want the multicast
answer, and it's fine (and useful) if others on the link overhear it
too.
""".
...
-doc """
Like `extract_answers/1`, but also includes the Authority section -
where RFC 6762 puts a prober's tentatively-claimed records. Used only
for probe/conflict watching (`mdns_socket`); these are proposals, not
...
%% of Unicode codepoints - two lists that share the same Erlang type but
%% need different treatment. to_binary/1's list clause
%% (unicode:characters_to_binary/1) is correct for a *caller-supplied*
%% Erlang string - genuinely a codepoint list - but would reinterpret
%% each raw byte as its own codepoint here, corrupting any multi-byte
%% UTF-8 character already on the wire. list_to_binary/1 just packs each
%% byte as-is, which is what's needed: the bytes are already
%% however-encoded (UTF-8, per RFC 6763) and must be preserved, not
%% reinterpreted.
...
a literal `\` becomes `\\`, a literal `.` becomes `\.`. Needed for
DNS-SD service instance names (RFC 6763 4.1.3), which are free-form
human-readable text and may contain either - unescaped, a `.` would be
misread as a label separator once the full name is built.
""".
-spec escape_label(string() | binary()) -> binary().
escape_label(Label) ->
    %% Must run in this order (backslash first): binary:replace/4 only
    %% scans the *original* input on each pass, so this can't double-escape
    %% a "." inserted while escaping a "\", or vice versa.
...
-> `~"my printer._http._tcp.local"`. InstanceName is escaped (see
`escape_label/1`), then the whole result is lowercased (via
`normalize_name/1`) - display casing isn't preserved, a known
...
%% Kept here, not in mdns_cache: interpreting what a record type's data
%% means on the wire is this module's job everywhere else too.
-doc """
Describe Data for a human, keyed off Type - used by
`mdns_cache:print/1`. Returns either `{inline, IoData}` for one line,
or `{multiline, [IoData]}` for a TXT record with more than one string.
Anything this app doesn't have a specific reading for - an
unrecognized type, or data that doesn't match a known type's expected
shape - falls back to a plain `~0p` dump rather than guessing wrong or
crashing.
""".
...
-doc """
A readable name for a DNS RR type (e.g. `nsec`, `ptr`), whether Type
is already an atom `inet_dns` recognized or still a bare integer -
NSEC's type bitmap (see `describe_nsec/1`) always uses bare integers,
regardless of whether `inet_dns` would otherwise recognize them. Falls
back to the integer itself, as text, for a type this doesn't have a
name for.
""".
...
-doc """
Build a classic unicast-DNS response for the bridge server, answering
the query in Req with `Answers :: [{Data, Ttl}]` for the given Type.
Found controls the response code: NXDOMAIN when nothing was found and
Answers is empty, NOERROR otherwise (including a query this app has no
opinion on, answered with an empty answer section).
""".
